...
ENV         FFIELD=amber  \
            ...
            CM_SIZEOF_F77_INTEGER=4 \
            F2C_INTEGER_C_TYPE=int \
            ...
            MA_USE_CUDA_MEM=1 \
            CUDA=nvcc \
            CUDA_FLAGS="-arch sm_87 " \
            CUDA_ARCH="-arch sm87" \
            CUDA_VERSION_MAJOR=11 \
            PATH=/usr/bin:/bin:/usr/sbin:/sbin:/usr/local/cuda-"$CUDA_VERSION_MAJOR"."$CUDA_VERSION_MINOR"/bin:/opt/nwchem/bin \
            LD_LIBRARY_PATH=/usr/local/cuda-"$CUDA_VERSION_MAJOR"."$CUDA_VERSION_MINOR"/lib64:/usr/local/lib64:$LD_LIBRARY_PATH \
            TCE_CUDA=Y \
